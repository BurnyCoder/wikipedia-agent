...
class WikipediaAgent:
    ...
    def _create_rag_tool(self):
        ...
        for file_path in rag_files:
            logger.info(f"WikipediaAgent._create_rag_tool :: Loading RAG data from {file_path}")
        ...
        logger.info(f"WikipediaAgent._create_rag_tool :: Created {len(texts)} text chunks from RAG data")
        
        # Create FAISS database with documents
        db = FAISS.from_documents(texts, embeddings)
        logger.info("WikipediaAgent._create_rag_tool :: Created FAISS database with RAG data")
